...
r = b - A.dot(x_nach)
# создаем массив для графика изменения относительной невязки
graph = []

# номер итерации для построения графика
k = 0
# закидываем в graph текущий номер итерации и относительную невязку
graph.append([ lg.norm(A.dot(x_nach) - b)/lg.norm(b)])
# сохраняем значение функционала, чтобы потом проверить уменьшение
F = []
...
while True:
    ...
    x_nach += alpha * r
    # 4 задание - условие Петрова-Галеркина
